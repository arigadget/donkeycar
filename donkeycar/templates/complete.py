...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[] ):
    ...
    if camera_type == "stereo":
        ...
        if cfg.DONKEY_GYM:
            ...
            if detectPlatform() == 'tpu':
                # two video interface already assigned on Tinker Edge T
                cam = Webcam(image_w=cfg.IMAGE_W, image_h=cfg.IMAGE_H, image_d=cfg.IMAGE_DEPTH, iCam = 2)
            else:
    ...
    if cfg.TRAFFIC_SIGN_ENABLED:
        from donkeycar.parts.detect_traffic_sign import Detect_traffic_sign
        ts_model = 'models/' + cfg.TRAFFIC_SIGN_MODEL
        ts_label = 'models/' + cfg.TRAFFIC_SIGN_LABEL
        detect_ts = Detect_traffic_sign(model_path=ts_model, label_path=ts_label, tpu_no=0)
    ...
    if cfg.TRAFFIC_SIGN_ENABLED:
        from donkeycar.parts.control_traffic_sign import Control_traffic_sign
        controller_ts = Control_traffic_sign()       
        V.add(controller_ts, 
